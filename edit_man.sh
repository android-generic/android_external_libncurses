...
(*.[0-9]*)
	...
	if test ! -f /media/Work/nougat-x86/external/libncurses/man_alias.sed ; then
cat >>/media/Work/nougat-x86/external/libncurses/man_alias.sed <<-CF_EOF2
		s,@DATADIR@,$datadir,g
...
s,@TSET@,tset,g
CF_EOF2
		echo "...made /media/Work/nougat-x86/external/libncurses/man_alias.sed"
	fi
	...
	aliases=`sed -f $top_srcdir/man/manlinks.sed $inalias |sed -f /media/Work/nougat-x86/external/libncurses/man_alias.sed | sort -u; test $inalias = $nCurses && echo curses`
	...
	sed	-f /media/Work/nougat-x86/external/libncurses/man_alias.sed \
		< $i >$TMP
